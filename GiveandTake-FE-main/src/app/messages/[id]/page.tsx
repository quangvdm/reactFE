...
import Message from "@/types/objects/message";
...
const items: Message[] = [
    {
        ...
        "userId": 1,
        "createDate": "2023-10-01T13:55:00Z"
    },
    {
        "id": 2,
        "name": "You",
...
function ChatScreen() {
    ...
    const [connection, sendMessage] = useRealTimeMessaging();

    const user = useLocalStorage<User | null>("user", null)[0];
    const [message, setMessage] = useState("");
    const [messages, setmesssages] = useState(items);
    useEffect(() => {
        console.log("Connection: ", connection)
        if (connection) {
            connection.on("receive", (userId: any, message: string) => {
                console.log({ userId, message });
                setmesssages(prevMessages => [
                    ...prevMessages,
                    {
                        id: prevMessages.length + 1,
                        name: userId === user?.accountId ? "You" : "Unknown",
                        content: message,
                        userId: userId,
    ...
    return <>
        ...
        <div className="no-scrollbar max-h-full space-y-3.5 overflow-auto px-6 py-7.5">
            {messages.map((message) => {
                const isSender = Number(user?.accountId) != Number(message.userId);
                const formattedDate = format(message.createDate, "h:mmaaa");
                if (isSender)
                    return <div key={message.id} className="max-w-125">
                ...
                return <div key={message.id} className="ml-auto max-w-125">
