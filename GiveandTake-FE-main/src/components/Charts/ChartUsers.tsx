            ...
            <div className="flex flex-wrap items-start justify-between gap-3 sm:flex-nowrap">
                <div className="flex w-full flex-wrap gap-3 sm:gap-5">
                    ...
                    {series.map((item) => {
                        ...
                        if (!RegExp(/^[0-9A-Za-z]+$/).test(colorFill)) colorFill = `[${colorFill}]`;
                        return (
                            <div key={item.name} className="flex min-w-47.5">
                                <span className={`mr-2 mt-1 flex h-4 w-full max-w-4 items-center justify-center rounded-full border border-${colorFill}`}>
                                    <span className={`block h-2.5 w-full max-w-2.5 rounded-full bg-${colorFill}`}></span>
                                </span>
                                <div className="w-full">
                                    <p className={`font-semibold text-${colorFill}`}>{item.label}</p>
